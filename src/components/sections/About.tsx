    ...
    <section id="about" className="px-6 py-24 md:py-32">
      <div className="mx-auto max-w-6xl">
        <SectionHeading
          label="About"
          title="Who I Am"
          description="4+ years building scalable enterprise applications — Java Spring Boot, Angular, and ERP/CRM platforms."
        />
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex h-full min-h-[7.5rem] flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center md:min-h-[8rem] md:p-6">
                <p className="bg-gradient-to-r from-indigo-400 to-emerald-400 bg-clip-text text-3xl font-bold text-transparent md:text-4xl">
                  {stat.value}
                </p>
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-wrap gap-2">
              {industries.map((industry) => (
                <span
                  key={industry}
                  className="rounded-full border border-indigo-500/20 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300"
                >
                  {industry}
                </span>
              ))}
        ...
        <div className="grid items-stretch gap-5 sm:grid-cols-2">
          {keyStrengths.map((strength, index) => (
            <ScrollReveal
              ...
              className="h-full"
            >
              <div className="h-full rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition-colors hover:border-indigo-500/20">
